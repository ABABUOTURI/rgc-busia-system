      ...
      try {
        ...
        if (response.ok) {
          ...
          const totalExpenditure = records.reduce((sum: number, record: any) => {
            return (
              sum +
              Object.values(record.expenditure || {}).reduce(
                (expSum: number, val: any) => expSum + (val || 0),
                0
              )
            );
          }, 0);
          ...
          const recentActivities = records.slice(0, 5).map((record: any) => {
            const totalOfferings =
              (record.offerings?.mainService || 0) +
              (record.offerings?.hbc?.jerusalem || 0) +
              (record.offerings?.hbc?.emmanuel || 0) +
              (record.offerings?.hbc?.ebenezer || 0) +
              (record.offerings?.hbc?.agape || 0) +
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div
          className={`p-3 sm:p-4 bg-white/70 backdrop-blur rounded-2xl shadow ${
            data.netAmount >= 0 ? "border-green-200" : "border-red-200"
          }`}
        >
          ...
          <div
            className={`text-lg sm:text-xl lg:text-2xl font-bold break-words ${
              data.netAmount >= 0 ? "text-black" : "text-black"
            }`}
          >
      ...
      {/* Recent activities & quick actions (unchanged) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
        <div className="p-3 sm:p-4 bg-white rounded-2xl shadow">
          <h4 className="font-semibold mb-2 text-sm sm:text-base text-gray-700">
            Recent Activities
          </h4>
